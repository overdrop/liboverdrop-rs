...
pub fn scan<BdS: AsRef<path::Path>, BdI: IntoIterator<Item=BdS>, Sp: AsRef<path::Path>, As: AsRef<str>>(
    base_dirs: BdI,
    shared_path: Sp,
    ...
    for dir in base_dirs {
        ...
        for entry in dir_iter.flatten() {
            ...
            if !allowed_extensions.is_empty() {
                if let Some(extension) = fpath.extension().and_then(|ext| ext.to_str()) {
